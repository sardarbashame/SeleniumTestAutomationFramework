...
public class TCCreateNewBusinessHO3SpecailFormAmlinPolicyWindHailExcludedTX extends BaseTest {
	...
	public void createQuote() throws InterruptedException,NullPointerException, IOException {
		...
		loginPO.LoginApp(
				getParameters().getProperty("UAT_ARYAUSERNAME"), 
				getParameters().getProperty("UAT_ARYAPASSWORD")
				);
		...
		createNewOpportunity.createNewOpp(
				...
				getParameters().getProperty("UAT_RETAILER"),
				getParameters().getProperty("UAT_RETAILCONTACT"),
				getParameters().getProperty("UAT_ORGACCOUNT")
				);
		
		AddRiskLocation addriskloc = new AddRiskLocation(driver);
		addriskloc.addRiskLoc(getParameters().getProperty("RISKLOCHarrisCountyTX"));
		markStageasComplete();
		Thread.sleep(1000);
		...
		risDetailsStep.riskdetailstep(getParameters().getProperty("AMLINCOVALIMIT"), getParameters().getProperty("AOPDEDUTIBLEPERCENTAGE"), getParameters().getProperty("WINDHAILDEDUTIBLETypeEXCLUDED")); 

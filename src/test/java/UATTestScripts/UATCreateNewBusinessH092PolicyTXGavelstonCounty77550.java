...
public class UATCreateNewBusinessH092PolicyTXGavelstonCounty77550 extends BaseTest {
	...
	public void createQuote() throws InterruptedException,NullPointerException, IOException {
		...
		loginPO.LoginApp(getParameters().getProperty("UAT_ARYAUSERNAME"), getParameters().getProperty("UAT_ARYAPASSWORD"));
		...
		createNewOpportunity.createNewOpp(getParameters().getProperty("DEPARTMENT"), getParameters().getProperty("HOMELOB"),getParameters().getProperty("PRODTEAM"),getParameters().getProperty("UAT_RETAILER"),getParameters().getProperty("UAT_RETAILCONTACT"),getParameters().getProperty("UAT_ORGACCOUNT"));
		AddRiskLocation addriskloc = new AddRiskLocation(driver);
		addriskloc.addRiskLoc(getParameters().getProperty("RISKLOCTXGavelstonCounty"));
		markStageasComplete();
		 Thread.sleep(1000);
		...
		risDetailsStep.riskdetailstep(getParameters().getProperty("H092COVALIMIT"), getParameters().getProperty("AOPDEDUTIBLEPERCENTAGE"), getParameters().getProperty("WINDHAILDEDUTIBLEEXCLUDED"));		 
